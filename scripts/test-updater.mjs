...
assert.equal(versionData.build, 'v8', 'build must be v8');
assert.equal(versionData.version, '1.8.0', 'version must be 1.8.0');

// 2. Verify sw.js
const swPath = path.resolve('public/sw.js');
assert.ok(fs.existsSync(swPath), 'public/sw.js must exist');
const swContent = fs.readFileSync(swPath, 'utf8');
assert.match(swContent, /VERSION\s*=\s*['"]v8['"]/, 'sw.js must define VERSION = v8');
...
assert.match(indexContent, /name=["']app-version["']\s+content=["']1\.8\.0["']/, 'index.html must have app-version meta tag');
assert.match(indexContent, /name=["']app-build["']\s+content=["']v8["']/, 'index.html must have app-build meta tag');
...
console.log('PASS: PWA updater, sw.js v8, version.json, and i18n verified successfully');
