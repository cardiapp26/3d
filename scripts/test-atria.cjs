...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(await visible(), ['la','laa','ra']);
    assert.equal(await page.locator('#structure-select').inputValue(), 'laa');
    assert.equal(await page.locator('#structure-select option:not([disabled])').count(), 3);
    for (const id of ['la','ra','laa']) {
    ...
    console.log('PASS: atrial isolation, deep link, focus, wall cuts, mode restoration and mobile reset');
